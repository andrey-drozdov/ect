		...
		if (this.ectad.options.cache && this.ectad.options.cache[template]) {
			...
			if (Object.prototype.toString.call(this.ectad.options.root) === '[object String]') {
				if (typeof process !== 'undefined' && process.platform === 'win32') {
					file = EctHelper.normalizePath((this.ectad.options.root.length && template.charAt(0) !== '/' && template.charAt(0) !== '\\' && !/^[a-zA-Z]:/.test(template) ? (this.ectad.options.root + '/') : '') + template.replace(this.extExp, '') + this.ectad.options.ext);
				} else {
					file = EctHelper.normalizePath((this.ectad.options.root.length && template.charAt(0) !== '/' ? (this.ectad.options.root + '/') : '') + template.replace(this.extExp, '') + this.ectad.options.ext);
				}
			} else {
				file = template;
			}
